...
## Status/badge tints (added while building the Admin dashboard)
- `--color-tint-bg`: rgba(158, 207, 212, 0.25) — light teal tint for badges/pills (category tag, theme pill, "recommended action" callouts)
- `--color-success-bg`: rgba(34, 197, 94, 0.08) — light tint of `--color-success` for "no issues" banners
- `--color-error-bg`: rgba(239, 68, 68, 0.08) — light tint of `--color-error` for "attention required" banners and issue badges
These aren't new hues — just low-opacity variants of tokens already above (same trick as `--shadow-card`), so status banners stay legible without reaching outside the 4 brand colors + success/error.
...
## Priority badges (added while building the AI Action Plan / Insights UI)
The Action Plan groups recommendations into Must Have / Should Have / Could Have / Watch (not the CRITICAL/HIGH/MEDIUM/LOW scale that's still used internally in the data). All four map to existing tokens, no new hues:
- **Must Have** → `--color-error-bg` bg, `--color-error` text (urgent, needs the strongest visual weight)
- **Should Have** → `--color-accent` bg, `--color-text-primary` text (our pale yellow-green — reads as "important, not urgent")
- **Could Have** → `--color-tint-bg` bg, `--color-primary` text (calm, optional)
- **Watch** → `--color-border` bg, `--color-text-muted` text (deliberately low-alarm — "monitor, no action needed yet")
See `components/Badge/Badge.jsx`.
